    ...
    func loginCheckOrRestart() {
        ...
        if isLoggedIn {
            ...
            if(UserDefaultsManager.shared.getUserType() == .Patient) {
                let homeVC = storyboard.instantiateViewController(withIdentifier: "PatientNavigation")
                window!.rootViewController = homeVC
            }
            
            if(UserDefaultsManager.shared.getUserType() == .Donor) {
                let homeVC = storyboard.instantiateViewController(withIdentifier: "DonorNavigation")
                window!.rootViewController = homeVC
            }
            
            if(UserDefaultsManager.shared.getUserType() == .Admin) {
                
                let homeVC = storyboard.instantiateViewController(withIdentifier: "AdminHomeVC")
                window!.rootViewController = homeVC
